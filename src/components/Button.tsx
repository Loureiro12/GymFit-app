import { Button as ButtonNativeBase, ButtonSpinner, ButtonText } from '@gluestack-ui/themed';
...
export function Button({ title, variant = 'solid', loading, disabled, ...rest }: Props) {
  return (
    <ButtonNativeBase
      ...
      {...rest}
    >
      {loading && <ButtonSpinner mr="$5" color={buttonTextColorVariation[variant]} />}
      <ButtonText fontSize="$md" fontWeight="$bold" color={buttonTextColorVariation[variant]}>
